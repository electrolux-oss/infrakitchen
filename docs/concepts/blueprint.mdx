...
**Blueprints** are reusable definitions that combine multiple Templates into a single, executable plan. A Blueprint describes which infrastructure components to provision and how their outputs wire together — turning a multi-resource stack into a one-click operation.
...
When executed, a blueprint creates a [Workflow](./workflow) that provisions each resource in the correct dependency order.
...
Go to **Blueprints** in the sidebar and click <kbd>Create</kbd>.
...
<Step title="Define wiring rules">
...
<Step title="Set default variables">
...
When a blueprint is executed, the system creates a [Workflow](./workflow) that orchestrates resource provisioning.
...
| **Template Version Overrides** | Use specific code versions per template           |
...
| **Variable Passing**  | Automated output → input               | Manual per resource             | Defined in the code repository   |
