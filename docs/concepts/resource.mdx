...
**Resources** are concrete instances of infrastructure created from **Templates** using specific **Template Versions**. While templates define what _can_ be built, resources represent _actual infrastructure_ running in your cloud environments.
...
- A **template version** (how to build it)
...
For the complete status reference, state constraints, and step-by-step workflows, see [Resource Workflows](resource-workflows).
...
| :---------------------- | :------------------------------------------ | :-------------------------------------------------- |
| **Name**                | Unique identifier for the resource          | Must be unique per template and template version |
...
| **Template Version**    | The version of IaC code to use              | Can be updated to new versions                      |
...
Go to **Resources** in the sidebar and click <kbd>Create</kbd>.

</Step>

<Step title="Select a template">

Pick the template to create the resource from.

</Step>

<Step title="Configure resource properties">

- Enter unique name
- Add description (optional)
- Select parent resources (if template has parents)
- Add dependency tags and configs (optional)

</Step>

<Step title="Configure template properties">
...
Fill in input variables based on the selected version.
...
<Step title="Follow standard creation steps 1–5">
...
<Step title="Skip infrastructure settings">

Not required for abstract resources.
...
For step-by-step processes, state transitions, and approval flows, see the [detailed workflow documentation](resource-workflows).
:::
...
Learn more about [Template](./template).

### Resources and Template Versions

Template versions contain the actual infrastructure code. Resources:

- Must select a template version to provision
...
See [Update Step - Temporary State Workflow](resource-workflows#temporary-state-workflow) for the complete process.
...
Learn more about [Kubernetes Integration](../../operations/kubernetes).
...
Learn more about [Audit Logging](../../operations/activity/audit).
