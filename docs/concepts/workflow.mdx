...
Open a Blueprint and click <kbd>Execute</kbd>.

</Step>

<Step title="Optionally configure runtime overrides">
...
<Step title="Select shared execution context">
...
Click <kbd>Create Execution</kbd>.
...
Variables for each step are resolved in the priority order described in [Blueprint — Variable Resolution Order](./blueprint#variable-resolution-order) (runtime overrides → wired outputs → blueprint defaults, highest wins).
:::
...
Wiring rules define how outputs from one template's resource feed into the inputs of another template's resource. The rules are defined on the [Blueprint](./blueprint#wiring-rules) and captured as an immutable snapshot when the workflow is created.
...
- ✅ Use a **Workflow** (via a [Blueprint](./blueprint)): "I need to provision a VPC, EKS cluster, and RDS database with automatic wiring" — dependency order, output passing, and parallel steps are computed for you.
...
For how Workflows relate to Templates and Executors, see the [comparison table](./blueprint#blueprints-vs-templates-vs-executors) on the Blueprint page.

## Best Practices

For blueprint design and variable management, see [Blueprint — Best Practices](./blueprint#best-practices).
