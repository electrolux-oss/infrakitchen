...
class BlueprintCRUD:
    ...
    async def create(self, data: dict[str, Any]) -> Blueprint:
        ...
        external_template_ids = data.pop("external_template_ids", [])
        blueprint = Blueprint(**data)
        self.session.add(blueprint)
        await self.session.flush()

        # Insert internal template association rows with position
        ...
        # Insert external template association rows
        for position, template_id in enumerate(external_template_ids):
            await self.session.execute(
                blueprint_templates.insert().values(
                    blueprint_id=blueprint.id,
                    template_id=template_id,
    ...
    async def update(self, blueprint_id: str | UUID, data: dict[str, Any]) -> Blueprint | None:
        ...
        external_template_ids = data.pop("external_template_ids", None)
        for key, value in data.items():
            if hasattr(blueprint, key):
                setattr(blueprint, key, value)

        if template_ids is not None or external_template_ids is not None:
            # Replace all template associations
            await self.session.execute(
                blueprint_templates.delete().where(blueprint_templates.c.blueprint_id == blueprint.id)
            )
            # Re-insert internal templates
            for position, template_id in enumerate(template_ids or [t.id for t in blueprint.templates]):
                await self.session.execute(
                    blueprint_templates.insert().values(
                        blueprint_id=blueprint.id,
                        template_id=template_id,
            ...
            # Re-insert external templates
            for position, template_id in enumerate(
                external_template_ids
                if external_template_ids is not None
                else [t.id for t in blueprint.external_templates]
            ):
                await self.session.execute(
                    blueprint_templates.insert().values(
                        blueprint_id=blueprint.id,
