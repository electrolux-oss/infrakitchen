...
class BlueprintConstant(BaseModel):
    id: uuid.UUID = Field(default_factory=uuid.uuid4)
    name: str = Field(...)
    type: Literal["string", "number"] | None = Field(default="string")
    defaultValue: Any = Field(default=None)


class BlueprintConfiguration(BaseModel):
    constants: list[BlueprintConstant] = Field(
        default_factory=list, description="List of constant variable names available in the blueprint"
    )
    constant_wires: list[WiringRule] = Field(
        default_factory=list,
        description="List of wiring rules that connect constant variables to template inputs",
    )
...
class BlueprintCreate(BaseModel):
    ...
    external_template_ids: list[uuid.UUID] = Field(
        default_factory=list,
        description="Template IDs for external (input) parent templates",
    )
...
class BlueprintUpdate(BaseModel):
    ...
    external_template_ids: list[uuid.UUID] | None = Field(default=None)
...
class BlueprintResponse(BaseModel):
    ...
    external_templates: list[TemplateShort] = Field(default_factory=list)
    wiring: list[WiringRule] = Field(default_factory=list)
