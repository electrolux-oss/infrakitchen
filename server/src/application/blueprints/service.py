...
class BlueprintService:
    ...
    async def create_workflow(
        ...
        ordered_template_ids = self._topological_sort(template_ids, wiring_rules)

        # Build steps
        steps = []
        for position, tid in enumerate(ordered_template_ids):
    ...
    @staticmethod
    def _topological_sort(template_ids: list[UUID], wiring_rules: list[WiringRule]) -> list[UUID]:
        """
        Sort templates so that sources come before targets.
        Templates with no wiring dependencies keep their original order.
        """
