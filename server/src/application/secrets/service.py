...
class SecretService:
    ...
    async def create(self, secret: SecretCreate, requester: UserDTO) -> SecretResponse:
        ...
        revision_number = await self.revision_handler.handle_revision(new_secret)
        await self.audit_log_handler.create_log(
            new_secret.id, requester.id, ModelActions.CREATE, revision_number=revision_number
        )
    ...
    async def update(self, secret_id: str, secret: SecretUpdate, requester: UserDTO) -> SecretResponse:
        ...
        revision_number = await self.revision_handler.handle_revision(existing_secret)
        await self.audit_log_handler.create_log(
            secret_id, requester.id, ModelActions.UPDATE, revision_number=revision_number
        )
