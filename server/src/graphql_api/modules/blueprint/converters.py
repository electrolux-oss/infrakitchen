...
def _convert_template_for_blueprint(obj: Any, include_parents: bool = False) -> TemplateType | None:
    """Convert a Template ORM object, optionally including parents."""
    base = convert_template_shallow(obj)
    if base is None:
        return None
    if include_parents:
        base.parents = [
...
def blueprint_options(
    ...
    Args:
        ...
        template_fields: Fields requested inside ``templates``.
    ...
    if "templates" in fields:
        tmpl_load = selectinload(Blueprint.templates)
        if template_fields and "parents" in template_fields:
            opts.append(tmpl_load.selectinload(Template.parents).noload("*"))
        else:
            opts.append(tmpl_load.noload("*"))
    if "external_templates" in fields or "externalTemplates" in fields:
        opts.append(selectinload(Blueprint.external_templates).noload("*"))
    if "workflows" in fields:
        ...
        if workflow_fields is None or "steps" in workflow_fields:
            ...
            if step_fields is None or "parentResourceIds" in step_fields:
                opts.append(step_load.selectinload(WorkflowStep.parent_resource_ids))
...
def convert_blueprint(
    ...
    if fields is not None:
        obj = SafeORM(obj)
    include_parents = template_fields is not None and "parents" in template_fields
    return BlueprintType(
        ...
        templates=(
            [
                x
                for x in (
                    _convert_template_for_blueprint(t, include_parents) for t in getattr(obj, "templates", []) or []
                )
                if x is not None
            ]
        ...
        external_templates=(
            [
                x
                for x in (convert_template_shallow(t) for t in getattr(obj, "external_templates", []) or [])
                if x is not None
            ]
            if fields is None or "external_templates" in fields or "externalTemplates" in fields
            else []
        ),
        wiring=obj.wiring,
