...
class BlueprintQuery:
    ...
    async def blueprint(self, info: Info, id: uuid.UUID) -> BlueprintType | None:
        ...
        stmt = select(Blueprint).where(Blueprint.id == id).options(*blueprint_options(fields))
        result = await session.execute(stmt)
        obj = result.scalars().unique().first()
        if obj is None:
            return None
        return convert_blueprint(obj, fields)
    ...
    async def blueprints(
        ...
        session = info.context["session"]
        fields = get_requested_fields(info)
        stmt = select(Blueprint).options(*blueprint_options(fields))
        ...
        result = await session.execute(stmt)
        return [x for x in (convert_blueprint(b, fields) for b in result.scalars().unique().all()) if x is not None]
