...
def secret_options(fields: set[str]) -> list[Any]:
    opts: list[Any] = build_load_only(Secret, fields)
    if "integration" in fields:
        opts.append(selectinload(Secret.integration).noload("*"))
    if "creator" in fields:
        opts.append(selectinload(Secret.creator))
    return opts


def convert_secret(obj: Any, fields: set[str] | None = None) -> SecretType | None:
    if obj is None:
        return None
    ...
    return SecretType(
        ...
        secret_provider=obj.secret_provider,
