...
from graphql_api.modules.blueprint.converters import convert_blueprint
...
def workflow_options(fields: set[str]) -> list[Any]:
    ...
    if "blueprint" in fields:
        opts.append(selectinload(Workflow.blueprint).selectinload(Blueprint.templates).noload("*"))
...
def convert_workflow(obj: Any, fields: set[str] | None = None) -> WorkflowType | None:
    ...
    return WorkflowType(
        ...
        blueprint=(
            convert_blueprint(obj.blueprint, {"id", "name", "templates", "wiring", "defaultVariables", "configuration"})
            if fields is None or "blueprint" in fields
            else None
        ),
        ...
        integration_ids=(
            [
                x
                for x in (
                    convert_integration(i, _NESTED_INTEGRATION_FIELDS)
                    for i in getattr(obj, "integration_ids", []) or []
                )
                if x is not None
            ]
