...
class TestCreate:
    ...
    @pytest.mark.asyncio
    async def test_create_integration_credentials_success(
        self,
        mock_integration_service,
        mock_integration_crud,
        mock_revision_handler,
        ...
        created_integration_body = {
            **integration_body,
            "created_by": mocked_user.id,
        }

        integration_create = Mock(spec=IntegrationCreate)
        integration_create.integration_type = "credentials"
        integration_create.model_dump = Mock(return_value=integration_body)

        new_integration = Integration(name="Test Integration", integration_type="credentials")
        mock_integration_crud.create.return_value = new_integration
        mock_integration_crud.get_by_id.return_value = Integration(name="Test Integration")
        ...
        monkeypatch.setattr(IntegrationResponse, "model_validate", Mock(return_value=integration_response))
        ...
        integration_create.model_dump.assert_called_once_with(by_alias=True, exclude={"_entity_name"})
        mock_integration_crud.create.assert_awaited_once_with(created_integration_body)

        assert new_integration.status == ModelStatus.ENABLED
        ...
        assert created_integration.name == "Test Integration"
        assert created_integration.integration_type == "credentials"
...
class TestValidateConnection:
    ...
    @pytest.mark.asyncio
    async def test_vault_integration_validate_connection_success(self, mock_integration_service, monkeypatch):
        configuration = {"vault_domain": "https://vault.example.com", "vault_token": "test_token"}

        mock_provider_instance = AsyncMock()
        mock_provider_instance.authenticate.return_value = None
        mock_provider_instance.is_valid.return_value = True
    ...
    @pytest.mark.asyncio
    async def test_vault_integration_validate_connection_fail(
        self, mock_integration_service, mock_integration_crud, monkeypatch
    ):
        configuration = {"vault_domain": "https://vault.example.com", "vault_token": "test_token"}

        mock_provider_instance = AsyncMock()
        mock_provider_instance.authenticate.return_value = None
        mock_provider_instance.is_valid.side_effect = CloudWrongCredentials("Invalid Vault credentials")
