...
class TestSourceCodeMutations:
    ...
    @pytest.mark.asyncio
    @patch("graphql_api.modules.source_code.mutations.get_source_code_service")
    async def test_update_source_code_omitted_fields_are_not_forwarded(
        self,
        mock_get_service,
        mock_source_code_service,
        mocked_source_code,
        ...
        assert result.errors is None
        assert mock_source_code_service.update_source_code.await_args
        forwarded = mock_source_code_service.update_source_code.await_args.kwargs["source_code"]
        # Only the explicitly provided field should be set; omitted fields must not be
        # forwarded so that model_dump(exclude_unset=True) does not clear them.
        assert forwarded.model_dump(exclude_unset=True) == {"description": "Updated description"}
        assert "integration_id" not in forwarded.model_fields_set
        assert "labels" not in forwarded.model_fields_set

    @pytest.mark.asyncio
    @patch("graphql_api.modules.source_code.mutations.get_source_code_service")
    async def test_update_source_code_explicit_null_is_forwarded(
        self,
        mock_get_service,
        mock_source_code_service,
        mocked_source_code,
        ...
        assert mock_source_code_service.update_source_code.await_args
        forwarded = mock_source_code_service.update_source_code.await_args.kwargs["source_code"]
        # An explicitly provided null must be preserved (distinct from omission).
        assert "description" in forwarded.model_fields_set
        assert forwarded.model_dump(exclude_unset=True) == {"description": None}

    @pytest.mark.asyncio
    @patch("graphql_api.modules.source_code.mutations.get_source_code_service")
    async def test_update_source_code_empty_labels_are_forwarded_to_clear(
        self,
        mock_get_service,
        mock_source_code_service,
        mocked_source_code,
        ...
        assert mock_source_code_service.update_source_code.await_args
        forwarded = mock_source_code_service.update_source_code.await_args.kwargs["source_code"]
        # An explicit empty list must be forwarded so labels are cleared (not treated as omission).
        assert "labels" in forwarded.model_fields_set
        assert forwarded.model_dump(exclude_unset=True) == {"labels": []}
