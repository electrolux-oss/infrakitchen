          ...
          if (unique.length > 0) {
            ...
            for (const w of unique) {
              const alreadyExists = merged.some(
                (e) =>
                  e.source_template_id === w.source_template_id &&
                  e.source_output === w.source_output &&
                  e.target_template_id === w.target_template_id &&
                  e.target_variable === w.target_variable,
              );
              if (!alreadyExists) merged.push(w);
            }
