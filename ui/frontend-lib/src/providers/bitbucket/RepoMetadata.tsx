  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, mb: 3 }}>
        {metadata.mainbranch && (
          <Chip
            label={`Default Branch: ${metadata.mainbranch.name}`}
            variant="outlined"
          />
        )}
        {metadata.language && (
        ...
        <Chip
          label={`Private: ${metadata.is_private ? "Yes" : "No"}`}
          variant="outlined"
        />
        {metadata.fork_policy && (
          <Chip
            label={`Fork Policy: ${metadata.fork_policy.replace(/_/g, " ")}`}
            variant="outlined"
          />
        )}
        {metadata.project && (
          <Chip
            label={`Project: ${metadata.project.name}`}
            variant="outlined"
          />
        )}
        {metadata.website && (
          ...
          >
            <Chip label="Website" variant="outlined" clickable />
          </MuiLink>
        )}
      </Box>
      {/* Owner Info */}
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        >
          ...
          <Typography variant="body2">
            <strong>Has Issues:</strong> {metadata.has_issues ? "Yes" : "No"}
          </Typography>
          <Typography variant="body2">
            <strong>Has Wiki:</strong> {metadata.has_wiki ? "Yes" : "No"}
          </Typography>
        </Grid>
        <Grid
          size={{
        ...
        >
          ...
          <Typography variant="body2">
            <strong>Created:</strong> {formatDate(metadata.created_on)}
          </Typography>
          <Typography variant="body2">
            <strong>Last Updated:</strong> {formatDate(metadata.updated_on)}
          </Typography>
        </Grid>
      </Grid>
      <Box sx={{ mt: 3, display: "flex", justifyContent: "center" }}>
        <MuiLink
          href={metadata.links.html.href}
          target="_blank"
          rel="noopener noreferrer"
          variant="button"
          sx={{
