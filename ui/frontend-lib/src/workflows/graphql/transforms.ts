...
export interface GqlWorkflow {
  ...
  parentOverrides: any;
  sourceCodeVersionOverrides: any;
  integrationIds: GqlIntegration[] | null;
  secretIds: GqlSecret[] | null;
  storageId: string | null;
  status: string;
...
export function transformWorkflow(gql: GqlWorkflow): WorkflowResponse {
  return {
    id: gql.id,
    blueprint_id: gql.blueprintId,
    blueprint: gql.blueprint
      ? {
          id: gql.blueprint.id,
          name: gql.blueprint.name,
          description: "",
          templates: (gql.blueprint.templates ?? []).map(toTemplateShort),
          wiring: gql.blueprint.wiring ?? [],
          default_variables: gql.blueprint.defaultVariables ?? {},
          configuration: gql.blueprint.configuration ?? {},
          labels: [],
    ...
    parent_overrides: gql.parentOverrides ?? {},
    source_code_version_overrides: gql.sourceCodeVersionOverrides ?? {},
    integration_ids: (gql.integrationIds ?? []).map(toIntegrationShort),
    secret_ids: (gql.secretIds ?? []).map(toSecretShort),
    creator: gql.creator
...
export function transformWorkflowListItem(
  ...
  return {
    id: g.id,
    blueprint_id: g.blueprintId,
    blueprint_name: g.blueprint?.name ?? null,
    status: g.status,
    error_message: g.errorMessage,
    creator: g.creator
